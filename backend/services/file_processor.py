...
class FileProcessor:
    ...
    Depending on the `material_type` different metadata should be extracted (see `CourseMaterialChunk` and `ExamQuestionChunk`)
    Currently, no metadata is extracted for the course material and only the question type is extracted for exam questions.
    ...
    Output:
        list[chunk]... The PDF text as a list of chunks
    """
    def chunk_and_enrich(self, pdf_file: BinaryIO, material_type: CourseMaterialType, course_id: int) -> list[CourseMaterialChunk] | list[ExamQuestionChunk]:
        pass